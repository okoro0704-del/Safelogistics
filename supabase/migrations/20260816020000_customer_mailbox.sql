-- Customer mailbox: threads linked to a customer profile with separate folder state.
...
-- Customers can access only their own threads / messages
...
COMMENT ON COLUMN public.email_threads.customer_id IS
  'Owning customer profile when this thread is part of the customer mailbox.';
COMMENT ON COLUMN public.email_threads.customer_folder IS
  'Customer mailbox folder: inbox | sent | drafts | spam';
