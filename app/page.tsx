...
export default async function LandingPage() {
  ...
  const displayName = ctx.tenant?.company_name ?? PLATFORM_DEFAULTS.appName;
  const tagline = ctx.isCustomDomain
    ? ctx.brand.tagline
    : PLATFORM_DEFAULTS.tagline;
        ...
        <section className="mx-auto grid w-full max-w-6xl gap-10 px-4 py-14 md:grid-cols-[1.15fr_0.85fr] md:items-center md:px-6 md:py-20">
          <div className="space-y-6">
            ...
            <p className="max-w-lg text-base leading-relaxed text-muted-foreground md:text-lg">
              {tagline} Manage deliveries, monitor progress, and keep customers
              informed in real time — without exposing private account data.
            </p>
